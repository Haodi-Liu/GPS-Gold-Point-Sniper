...
class CapVidData(torch.utils.data.Dataset):
    def __init__(self, loaded_data, vis_processor, root_path):
        self.loaded_data = loaded_data
        self.key_list = list(loaded_data.keys())
        self.root_path = root_path
        self.vis_processor = vis_processor
    ...
    def __getitem__(self, idx):
        ...
        img_id = key.rsplit('.')[0] + '.jpg'
        #img_id = key.rsplit('.')[0] + '.mp4.jpg'
        image_path = os.path.join(self.root_path, img_id)
        image = Image.open(image_path).convert('RGB')
        image = self.vis_processor(image)
        return img_id, image, self.loaded_data[key]

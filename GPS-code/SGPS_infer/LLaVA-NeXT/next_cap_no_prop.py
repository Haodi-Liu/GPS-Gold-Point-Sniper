...
SQ = "Please propose 3-5 questions(each no more than 20 words) that would seek for more information in order to recognize the ongoing human action more accurately. The questions asked have to be related to the ongoing human action and can be answered with visually grounded evidences from the given video frames. Also be sure to leverage the temporal changes in the frames and never make any ungrounded assumptions in the questions."
...
for img in tqdm(img_lst):
    ...
    if store[img]['decom'].startswith('No'):
        store[img]['question'] = generate(SQ, conv_template)
        ...
        store[img]['summary'] = generate(SU_DIR, conv_template)
    
    else:
        print("Error: ", store[img]['decom'])

with open("./{}-noprop-perf-heldin.json".format(checkpoint.rsplit('/')[-1]), "w") as f:
    json.dump(store, f)